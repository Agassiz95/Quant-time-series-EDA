...
# === Step 4: Fit GARCH(1,1) to Returns ===
garch_model = arch_model(df['Log_Returns'] * 100, vol='Garch', p=1, q=1)
...
# === 12. Relative Volatility Features ===
...
# === 13. Subtract ARIMA(0,1,0) ===
model = ARIMA(df['Close'], order=(0, 1, 0)).fit()
df['arima_resid'] = model.resid

# === 14. Recalculate Indicators on Residuals ===
resid = df['arima_resid'].dropna()

# GARCH 
model = ARIMA(df['Log_Returns'], order=(0, 1, 0)).fit()
df['arima_resid'] = model.resid
garch_model = arch_model(df['arima_resid'] * 100, vol='Garch', p=1, q=1)
garch_fit = garch_model.fit(disp='off')
df['garch_volatility'] = garch_fit.conditional_volatility
...
plt.plot(df['kf_smooth'], label='Kalman Smoothed', linestyle='--')
plt.title(f"{ticker} - Price vs Kalman Smoothed")
plt.legend()

# Volatility
plt.subplot(2, 1, 2)
plt.plot(df['GARCH_Volatility'], color='orange')
plt.title(f"{ticker} - GARCH(1,1) Estimated Volatility (%)")
...
## ------------------------------------------------- ##
## ---------- GARCH after ARIMA(0, 1, 0) ----------- ##
## ------------------------------------------------- ##

plt.figure(figsize=(12, 4))
plt.plot(df['garch_volatility'], color='darkorange')
plt.title("Conditional Volatility from GARCH(1,1) on ARIMA(0,1,0) Residuals")
